...
export const sistemaRecomendaciones = () => {
  /**
   ...
   */
  const calcularSimilitudUsuarios = (idUsuario1 = 1, idUsuario2 = 1, pondHobbie = 0.3, pondCategorias = 0.4, pondEdad = 0.15, pondCiudad = 0.15) => {
    ...
    const hobbiesComunes = [...hobbies1].filter((hobbie) =>
      hobbies2.has(hobbie)
    );
    ...
    const similitudCategorias =
      comunesCategories.length /
      Math.max(categories1.length, categories2.length);

    // edad
    const maxDiferencia = 50;
    const diferenciaEdad = usuario1.edad - usuario2.edad;
    ...
    const similitudTotal =
      similitudHobbies * pondHobbie +
      similitudCategorias * pondCategorias +
      similitudEdad * pondEdad +
      similitudCiudad * pondCiudad;
   ...
   */
  const obtenerUsuariosSimilares = (idUsuario = 1, limite = 1) => {
    ...
    return usuarios
      ...
      .sort(
        (primerUser, segundoUser) =>
          segundoUser.similitud - primerUser.similitud
      )
      .slice(0, limite);
  };
   ...
   */
  const generarRecomendaciones = (idUsuario = 1, numeroRecomendaciones = 5) => {
    ...
    todosProductos.forEach((p) => {
      ...
      recomendacionesMap.set(p.id, {
        ...
        categoriaRelacionada: categoriasUsuario.includes(p.categoría)
          ? p.categoría
          : 'No tiene categorías relacionadas',
      });
    });
   ...
   */
  const obtenerEstadisticasRecomendaciones = () => {
    ...
    const promedioValoracion = totalProductosRecomendables
      ? productosRecomendables.reduce((total, product) => total + product.valoracion, 0) / totalProductosRecomendables
      : 0;
   ...
   */
  const actualizarRecomendaciones = (idUsuario = 1) => {
   ...
   */
  const explicarRecomendacion = (idUsuario = 1, idProducto = 101) => {
    const usuario = usuarios.find((u) => u.id === idUsuario);
   ...
   * @param {number} idUsuario - ID del usuario objetivo
   */
  const guardarRecomendacionesLocalStorage = (idUsuario = 1) => {
  ...
  const demostracionEjercicio21 = () => {
    ...
    console.table(calcularSimilitudUsuarios(4, 2));
...
export default sistemaRecomendaciones;
