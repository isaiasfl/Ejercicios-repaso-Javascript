...
const carritoCompras = () => {
   ...
   */
  const agregarProducto = (idProducto, cantidad=1) => {
    try{
  ...
  const actualizarCantidad = (idProducto, nuevaCantidad) => {
    ...
    const carrito = JSON.parse(localStorage.getItem('carrito'));
    const producto = carrito.find(product => product.idProducto === idProducto);
    if(!producto){
   ...
   * @param {number} codigoDescuento - Código de descuento a aplicar.
   */
  const aplicarCodigoDescuento = (codigoDescuento) => {
    if(!localStorage.hasOwnProperty('carrito')){
      return 0;
    }
    ...
    const carrito = JSON.parse(localStorage.getItem('carrito'));
    // a cada producto se le aplica un 12% de descuento en el subtotal
    const carritoConDescuento = carrito.map(producto => {
      const subtotalConDescuento = producto.subtotal * 0.88;
      return {
        ...producto,
   ...
   * @param {number} costePorcentual - Porcentaje de gastos de envío sobre el total del carrito. Por defecto es 1 (1% del total del carrito).
   * @returns - Retorna el total con los gastos de envío incluidos.
   */
  const calcularGastosEnvio = (costePorcentual = 1) => {
    const gastosEnvio = calcularTotal() * ( 1 + costePorcentual/100 || 1.01);
    return gastosEnvio;
    
  };

  /**
   * Esta función simula el proceso de checkout.
   */
  const simularProcesoCheckout = () => {
  ...
  const demostracionEjercicio14 = () => {
    ...
    console.log('Carrito después de agregar, eliminar y actualizar cantidades:');
    console.log(obtenerCarrito());
    console.log(calcularTotal());

    vaciarCarrito();

    console.log('Carrito después de vaciarlo:');
    console.log(obtenerCarrito());
    ...
    agregarProducto(104,2);
    agregarProducto(105,1);
    console.log('Carrito para probar código de descuento:');
    console.log(obtenerCarrito());
    console.log(calcularTotal());

    console.log('Total con descuento aplicado:');
    aplicarCodigoDescuento(24214);
    console.log(calcularTotal());

    console.log('Gastos de envío:');
    console.log(calcularGastosEnvio());
    
    simularProcesoCheckout();
  };
  return {
    agregarProducto, eliminarProducto, actualizarCantidad, vaciarCarrito, obtenerCarrito, calcularTotal, aplicarCodigoDescuento, calcularGastosEnvio, simularProcesoCheckout, demostracionEjercicio14
  };
